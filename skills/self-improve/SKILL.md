...
| Step | Role | OMC Agent (falls back to, standalone) | Model |
|------|------|-----------|-------|
| Research | Codebase analysis + hypothesis generation | general-purpose Agent | opus |
| Planning | Hypothesis → structured plan | oh-my-claudecode:planner (→ `pantheon:researcher`) | opus |
| Architecture Review | 6-point plan review | oh-my-claudecode:architect (→ `pantheon:reviewer`) | opus |
| Critic Review | Harness rule enforcement | oh-my-claudecode:critic (→ `pantheon:reviewer`) | opus |
| Execution | Implement plan + run benchmark | oh-my-claudecode:executor (→ `pantheon:worker`) | opus |
| Git Operations | Atomic merge/tag/PR | oh-my-claudecode:git-master (→ `pantheon:worker`) | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `oh-my-claudecode:git-master` (when the OMC engine is installed; standalone falls back to `pantheon:worker`, then `general-purpose`):
  ```
...
Spawn N `oh-my-claudecode:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings). when the OMC engine is installed; standalone falls back to `pantheon:researcher`, then `general-purpose`.
...
**6a. Architecture Review**: Spawn `oh-my-claudecode:architect` (when the OMC engine is installed; standalone falls back to `pantheon:reviewer`, then `general-purpose`) with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `oh-my-claudecode:critic` (when the OMC engine is installed; standalone falls back to `pantheon:reviewer`, then `general-purpose`) with the plan + harness rules:
...
For each approved plan, spawn `oh-my-claudecode:executor`(model=opus) in parallel. when the OMC engine is installed; standalone falls back to `pantheon:worker`, then `general-purpose`.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `oh-my-claudecode:git-master` (when the OMC engine is installed; standalone falls back to `pantheon:worker`, then `general-purpose`): `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
