...
# atropos — cut the thread on any running mode
...
`/pantheon:atropos` follows the session-aware state contract:
...
```
/pantheon:atropos --force
```

```
/pantheon:atropos --all
```
...
3. If a `session_id` was supplied to `/pantheon:atropos`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
