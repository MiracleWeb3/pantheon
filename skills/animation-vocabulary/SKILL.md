...
description: Reverse-lookup glossary that turns a vague description of a web animation into its exact term ("the bouncy popover open" -> Pop in). Use when the user asks "what's it called when...", or describes a motion effect without knowing its name. For naming an effect, not building one.
---
