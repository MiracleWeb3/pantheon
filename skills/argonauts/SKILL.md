...
# argonauts — a crew of agents on one task list
...
```
/pantheon:argonauts N:agent-type "task description"
/pantheon:argonauts "task description"
/pantheon:argonauts ralph "task description"
```
...
- **Cancel:** `/pantheon:atropos` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omc/handoffs/` are preserved for potential resume.
...
1. User invokes `/team ralph "task"` or `/pantheon:argonauts ralph "task"`
...
4. If architect approves: both modes complete, run `/pantheon:atropos`
...
The `/pantheon:atropos` skill handles team cleanup:
...
3. Or run `/pantheon:atropos` which handles OMC state cleanup automatically.
