...
> *pantheon-native name — this skill ships as `omc-doctor` in its source (see CREDITS.md for attribution).*
...
`ai-slop-cleaner`, `ask`, `automedon`, `cancel`, `triumvirate`, `configure-notifications`, `deep-interview`, `atlas-init`, `external-context`, `engine-hud`, `forge-session`, `learner`, `mcp-setup`, `engine-doctor`, `engine-install`, `engine-teams`, `plan`, `project-session-manager`, `sisyphus`, `sisyphus-plan`, `release`, `archimedes`, `engine-setup`, `skill`, `team`, `hekaton-qa`, `hekaton`, `visual-verdict`, `writer-memory`
