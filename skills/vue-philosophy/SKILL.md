...
> *pantheon-native name — this skill ships as `antfu` in its source (see CREDITS.md for attribution).*
...
Use [`fast-npm-meta`](https://github.com/vue-philosophy/fast-npm-meta) to look up the latest version of a package — it queries a small metadata endpoint instead of downloading the full registry payload (which can be megabytes per package).
...
```js
// eslint.config.mjs
import vue-philosophy from '@vue-philosophy/eslint-config'

export default vue-philosophy()
```


When completing tasks, run `pnpm run lint --fix` to format the code and fix coding style.

For detailed configuration options: [vue-philosophy-eslint-config](references/vue-philosophy-eslint-config.md)
...
|-------|-------------|-----------|
| ESLint Config | Framework support, formatters, rule overrides, VS Code settings | [vue-philosophy-eslint-config](references/vue-philosophy-eslint-config.md) |
