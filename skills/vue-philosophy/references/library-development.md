...
| Tool | Purpose |
|------|---------|
| [`tsnapi`](https://github.com/vue-philosophy/tsnapi) | Snapshots runtime exports + type declarations into committed `.snapshot.js` / `.snapshot.d.ts` files via Vitest |
| [`tsdown-stale-guard`](https://github.com/vue-philosophy-collective/tsdown-stale-guard) | Records build input/output hashes so tests fail fast when run against a stale build |
