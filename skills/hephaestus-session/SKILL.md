...
aliases: [learner]
description: Extract a repeatable workflow from the CURRENT session into a reusable skill draft (the session-mining variant of forge, which authors disciplines from scratch).
---


> *pantheon-native name — this skill ships as `skillify` in its source (see CREDITS.md for attribution).*

# hephaestus-session — extract a skill from what just happened

Use this skill when the current session uncovered a repeatable workflow that should become a reusable OMC skill.

> Compatibility: `/pantheon:mathesis` is a deprecated alias for this skill. Prefer `/pantheon:hephaestus-session` in docs, prompts, and new workflows. Internal implementation modules may still use the learner name.
