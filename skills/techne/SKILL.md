...
# techne — manage the local skills
...
2. Scan user skills at `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/pantheon-learned/`
...
USER SKILLS (~/.claude/skills/pantheon-learned/):
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/pantheon-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.claude/skills/pantheon-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/pantheon-learned/<name>/SKILL.md`
   - `.omc/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/pantheon-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.claude/skills/pantheon-learned/)
...
```
Skill: custom-logger
Scope: user (~/.claude/skills/pantheon-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/pantheon-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.claude/skills/pantheon-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/pantheon-learned/`
   - Project skills: `.omc/skills/`
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/pantheon-learned"
...
echo "=== USER-LEVEL SKILLS (~/.claude/skills/pantheon-learned/) ==="
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/pantheon-learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/pantheon-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/pantheon-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Then ask for scope:
- **User-level** (~/.claude/skills/pantheon-learned/) - Available across all projects
...
- `/pantheon:techne list` - Show detailed skill inventory
- `/pantheon:techne add` - Start skill creation (invoke skillify)
- `/pantheon:techne scan` - Scan both skill directories
...
- `/pantheon:hephaestus-session` - Extract a skill from current conversation (`/pantheon:mathesis` is a deprecated alias)
- `/pantheon:note` - Save quick notes (less formal than skills)
- `/pantheon:atlas-init` - Generate AGENTS.md codebase hierarchy
...
```
> /pantheon:techne list

Checking skill directories...
✓ User skills directory exists: ~/.claude/skills/pantheon-learned/
...
- Run `/pantheon:techne list` periodically to review your skill library
